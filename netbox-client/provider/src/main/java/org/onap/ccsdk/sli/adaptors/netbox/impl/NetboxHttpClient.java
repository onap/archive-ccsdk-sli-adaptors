...
public class NetboxHttpClient implements AutoCloseable {
    ...
    public CompletionStage<HttpResponse> post(final String url, final String requestBody, final String token) {
        return sendRequest(url, token, requestBody, HttpPost::new);
    }

    public CompletionStage<HttpResponse> delete(final String url, final String token) {
        return sendRequest(url, token, HttpDelete::new);
    }
    ...
    private <T extends HttpUriRequest> CompletionStage<HttpResponse> sendRequest(final String url, final String token,
        final Function<String, T> supplier) {
        final T request = supplier.apply(url);
    ...
    CompletionStage<HttpResponse> sendRequest(final String url, final String token,
        final String body,
        final Function<String, T> supplier) {
        final T request = supplier.apply(url);
        request.addHeader(ACCEPT, APPLICATION_JSON);
